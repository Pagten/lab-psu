...
#define SPI_SET_MODE(cpol,cpha)  ( SPCR = cpol ? SPCR | _BV(CPOL) : SPCR & ~_BV(CPOL); SPCR = cpha ? SPCR | _BV(CPHA) : SPCR & _BV(CPHA); )
#define SPI_SET_CLOCK_RATE_div_4 ( SPCR &= ~(_BV(SPI2X) | _BV(SPR1) | _BV(SPR0)) )
